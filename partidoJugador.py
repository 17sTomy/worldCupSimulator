...
def goalCounter(intentos, golesAFavor, golesEnContra, diccionario, miEquipo, rival):
    """
    Juega el turno y cuenta los goles que se van convirtiendo
    """
    while intentos > 0:
...
def partidoPlayer(miEquipo, rival, diccionario, tipoPartido): 
    ...
    ganador = None
    golesAFavor, golesEnContra = goalCounter(3, 0, 0, diccionario, miEquipo, rival)
    ...
    if golesAFavor == golesEnContra: #si hay empate y el partido es de tipo eliminatorias, se simulan penales
        if tipoPartido == "grupo":
            ...
            print()  
            golesAFavor, golesEnContra = goalCounter(1, golesAFavor, golesEnContra, diccionario, miEquipo, rival)
            if golesAFavor == golesEnContra:
                print("NOS VAMOS A LOS PENALES...")
                time.sleep(1)
                resultadoRandom1 = random.randint(0, 5)
                resultadoRandom2 = random.randint(0, 5)
                while resultadoRandom1 == resultadoRandom2 or abs(resultadoRandom1 - resultadoRandom2) > 3:
                    resultadoRandom1 = random.randint(0, 5)
                    resultadoRandom2 = random.randint(0, 5)
                print()
